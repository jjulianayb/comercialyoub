...
function Icon({ label }: { label: string }) {
  const variants = [
    ...
    <><path d="M5 19V5h14v14" /><path d="M8 9h8M8 13h5" /></>,
    <><path d="M4 17l5-5 3 3 5-7 3 3" /><circle cx="17" cy="8" r="1" /></>,
  ];
  const index = Array.from(label).reduce((total, char) => total + char.charCodeAt(0), 0) % variants.length;
  return <span className="proposal-vector-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">{variants[index]}</svg></span>;
}
...
export function PropostaInterativa({
  ...
  return (
    <div className="proposta proposta-locked proposal-dynamic">
      ...
      <section className="sec"><span className="eyebrow">01 — Contexto</span><h2 className="sec-h">Onde a <span>{content.companyName}</span> está</h2><p className="sec-sub">Um ponto de partida claro para transformar cultura, liderança e desenvolvimento.</p><div className="ctx-box"><p>{content.context}</p><div className="ctx-pills">{content.metrics.map(([icon, label]) => <span key={label} className="pill"><Icon label={icon} />{label}</span>)}</div></div><div className="nums-grid">{content.numbers.map(([number, label]) => <div key={label} className="num-card"><div className="n">{number}</div><div className="l">{label}</div></div>)}</div></section>
      ...
      <section className="sec"><span className="eyebrow">03 — Escopo</span><h2 className="sec-h">Frentes de <span>atuação</span></h2><p className="sec-sub">Estrutura modular para desenvolver lideranças, pessoas e a cultura da organização.</p><div className="escopo-grid">{content.fronts.map(([icon, title, description]) => <article key={title} className="escopo-card"><div className="escopo-ico"><Icon label={icon} /></div><h4>{title}</h4><p>{description}</p></article>)}</div></section>
